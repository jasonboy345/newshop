<?php 
    ...
    //获取当前父类别id（也就是当前类别的，父id(pid)的获取）
    $pid = $_GET['pid']+0;  //当拿不到值时'+0'是这么来的
    
    //判断是否在根类别下(好好的推敲一下)
    if($pid>0){
        //根据当前父类别id号获取对应的父类别path信息
        $sql = "select path from type where id = {$pid}";
        //echo $sql;
        //exit(); 
        $res = mysqli_query($link,$sql);
        $path = mysqli_num_rows($res); 
        //echo $path;
        //exit();
        //获取当前位置上的所有路径(类似于:0,)中类别信息列表
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top" id="addinfo">您的位置：<a href="index.php">类别管理</a><a href="newindex1.php">>>分层浏览</a></td>
  </tr>
  <tr>
    <td align="left" valign="top">
     <div style="width:600px;text-align:left;">
        <a href='newindex1.php?pid=0'>根类别</a> ->
        <?php
            //判断并遍历输出路径信息
            if($typeres && mysqli_num_rows($typeres)>0){
                while($row = mysqli_fetch_assoc($typeres)){
                    echo "<a href=\"newindex1.php?pid={$row['id']}\">{$row['name']}</a> ->";
                }
            }
        ?>
    </div>    
      ...
      <?php
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                $nbsp = str_repeat("&nbsp;",$m*4);  //计算空格
                  ...
                  echo "<td align=\"left\" valign=\"middle\" class=\"borderright borderbottom tdb\" style=\"width: 120px;\" ><a href='newindex1.php?pid={$row['id']}'>->{$row['name']}</a></td>";
                  ...
                  echo "<td align=\"center\" valign=\"middle\" class=\"borderbottom\">
                        ...
                        <a href=\"javascript:liu({$row['id']})\" target=\"mainFrame\" onFocus=\"this.blur()\" class=\"add\"><input name=\"button\" type=\"button\" value=\"删除\"/></a></td>";
                  echo "</tr>";
